...
class Pagination extends React.Component {
	...
	handleGoPage(){
		let goNum = this.state.num;
		let goNum_f = Number(goNum);
		let {total} = this.props;
		if (goNum!=""&&total>=goNum_f>0) {
			
			this.handlePageChanged(goNum_f-1);
			this.setState({num: ""});
		}else{
			alert("请输入1到"+total+"之间的数字");
    ...
    handlePageChanged(select, e){
        ...
        this.props.onPageChanged(select);
	};
	...
	renderPages(){
        ...
        if (total <= visiblePages) {
            ...
            for (let index = 0; index < total; index++) {
				items.push(this.pageInit(index));
            }
			...
			<ul className={cointerWrapClass}>
				{showFirstAndLast&&
					<li className={previousClasses} 
						onClick={this.handlePageChanged.bind(this, 0)}>
						<a href="javascript:;">{firstBtn}</a>
					</li>
				}
				<li className={previousClasses}>
					<a
				...
				{showFirstAndLast&&
					<li  className={nextClasses}
						onClick={this.handlePageChanged.bind(this, Number(total)-1)}>
						<a href="javascript:;">{lastBtn}</a>
					</li>
				}
				{this.props.showGo&&
